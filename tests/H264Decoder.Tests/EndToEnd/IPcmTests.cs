...
public sealed class IPcmTests
{
    ...
    private sealed class TestCabacEncoder
    {
        ...
        private bool TryProduce(ulong candidate, int nbits, out int usedBits)
        {
            ...
            {
                sbyte m = H264Decoder.Cabac.CabacInitTable.MN[ctxIdx, 0, 0];
                sbyte n = H264Decoder.Cabac.CabacInitTable.MN[ctxIdx, 0, 1];
                if (m == H264Decoder.Cabac.CabacInitTable.CtxNA) continue;
                contexts.Initialize(ctxIdx, m, n, sliceQp);
            }
            var dec = new H264Decoder.Cabac.CabacDecoder(buf, 0, contexts);
